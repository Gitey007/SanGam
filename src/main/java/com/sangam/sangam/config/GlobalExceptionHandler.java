...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleMailException(MailException ex) {
        ...
        String rootCauseInfo = rootCause != null 
                ? "[" + rootCause.getClass().getName() + "] " + rootCause.getMessage()
                : "None";
        logger.error("MailException encountered while sending email: [{}] {}. Root cause: {}", 
                ex.getClass().getName(), ex.getMessage(), rootCauseInfo, ex);

        Map<String, Object> body = new HashMap<>();
        body.put("timestamp", LocalDateTime.now());
        body.put("status", HttpStatus.SERVICE_UNAVAILABLE.value());
        body.put("error", "Mail Service Error");
        body.put("message", "Unable to send email OTP. Please check SMTP configuration or try again later.");
    ...
    public ResponseEntity<Map<String, Object>> handleDataIntegrityViolationException(org.springframework.dao.DataIntegrityViolationException ex) {
        ...
        body.put("message", "A database conflict or duplicate record already exists.");
    ...
    public ResponseEntity<Map<String, Object>> handleGenericException(Exception ex) {
        ...
        body.put("message", ex.getMessage() != null ? ex.getMessage() : "An internal server error occurred");
