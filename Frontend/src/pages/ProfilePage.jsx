          ...
          {skillsList.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {skillsList.map((skill, index) => (
                <span
                  key={`${skill}-${index}`}
                  className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-800 border border-slate-200/80 hover:bg-slate-200/70 transition-colors"
                >
                  <span>{skill}</span>
                  {isOwnProfile && (
                    <button
                      ...
                      className="p-0.5 rounded-full hover:bg-slate-300/80 text-slate-400 hover:text-slate-700 transition-colors"
                      title={`Remove ${skill}`}
                    >
                      <X className="w-3 h-3" />
                    </button>
          ...
          <div className="mt-6 pt-5 border-t border-slate-100">
            ...
            >
              <input
                ...
                className="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-900 placeholder:text-slate-400 transition-all focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600"
              />
              <Button
                type="submit"
                variant="primary"
              ...
              {POPULAR_SKILLS.filter(
                ...
                .map((ps) => (
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-200 hover:bg-brand-50 hover:text-brand-600 hover:border-brand-200 transition-colors"
                  >
          ...
          {projectsList.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {projectsList.map((project) => (
                <div
                  key={project.id}
                  className="rounded-xl border border-slate-200 p-5 bg-slate-50/40 hover:bg-slate-50 hover:border-slate-300 transition-all flex flex-col justify-between"
                >
                  <div>
                    ...
                    {project.techStack && (
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {project.techStack.split(',').map((tech, i) => (
                          <span
                            key={`${tech}-${i}`}
                            className="px-2 py-0.5 rounded bg-white text-[11px] font-medium text-slate-600 border border-slate-200"
                          >
                            {tech.trim()}
                          </span>
                        ))}
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-200/60">
                    {project.githubUrl && (
                      <a
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-slate-200/70 hover:bg-slate-200 text-slate-700 text-xs font-medium transition-colors"
                      >
                    ...
                    {project.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-brand-50 hover:bg-brand-100 text-brand-700 text-xs font-medium transition-colors"
                      >
            ...
            <div className="text-center py-8 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
              ...
              <p className="text-xs text-slate-500 font-medium">No projects showcased yet.</p>
              {isOwnProfile && (
          ...
          {achievementsList.length > 0 ? (
            <div className="space-y-3">
              {achievementsList.map((ach) => (
                <div
                  key={ach.id}
                  className="rounded-xl border border-slate-200 p-4 bg-white hover:border-slate-300 transition-all flex flex-col sm:flex-row sm:items-start justify-between gap-3"
                >
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wider bg-amber-50 text-amber-800 border border-amber-200">
            ...
            <div className="text-center py-8 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
              ...
              <p className="text-xs text-slate-500 font-medium">No achievements added yet.</p>
              {isOwnProfile && (
                <p className="text-xs text-slate-400 mt-1">
