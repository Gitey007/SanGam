    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="relative flex items-center">
        <select
          ...
          onChange={onChange}
          disabled={disabled}
          className={`w-full appearance-none rounded-lg border bg-white dark:bg-slate-900 pl-3 pr-9 py-2 text-sm text-slate-900 dark:text-slate-100 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 disabled:bg-slate-50 dark:disabled:bg-slate-850 disabled:text-slate-500 disabled:cursor-not-allowed ${
            error
      ...
      {error && (
        <p className="mt-1.5 text-xs text-rose-600 dark:text-rose-400 font-normal">{error}</p>
      )}
